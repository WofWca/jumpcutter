...
const {
  minimizeJsonString: minimizeI18nMessagedJsonString,
} = require("minimize-webext-i18n-json");
...
module.exports = (env) => {
  if (!env.browser) {
    ...
    throw new Error(
      "You must define the `browser` environment variable (`--env browser=chromium`",
    );
  }
  const definePlugin = new webpack.DefinePlugin({
    ...
    "BUILD_DEFINITIONS.BROWSER_MAY_HAVE_AUDIO_DESYNC_BUG": JSON.stringify(
    ...
    "BUILD_DEFINITIONS.CONTACT_EMAIL": JSON.stringify("wofwca@protonmail.com"),
  });
  ...
  return {
    ...
    entry: {
      ...
      SilenceDetectorProcessor:
        "./src/entry-points/content/SilenceDetector/SilenceDetectorProcessor.ts",
      ...
      new CopyPlugin({
        patterns: [
...

          {
            ...
            transform: (content) =>
              minimizeI18nMessagedJsonString(content, { unsafe: false }),
          },
          ...
          {
            context: "src",
            from: "icons/(icon.svg-64.png|icon-disabled.svg-64.png|icon-only-sounded.svg-64.png|icon.svg-128.png|icon-disabled.svg-128.png|icon-only-sounded.svg-128.png|icon-big-padded.svg-128.png)",
          },
          {
            context: "src/entry-points",
            from: "popup/*.(html|css)",
            to: "popup/[name][ext]",
          },
          {
          ...
          {
            context: "src/entry-points",
            from: "local-file-player/*.(html|css)",
            to: "local-file-player/[name][ext]",
          },
        ],
      }),
