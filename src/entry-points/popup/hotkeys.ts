...
import {
  keydownEventToActions,
  eventTargetIsInput,
  NonSettingsActions,
} from "@/hotkeys";
import type { Settings } from "@/settings";

type RequiredSettings = Parameters<typeof keydownEventToActions>[1] &
  Pick<
    Settings,
    "popupDisableHotkeysWhileInputFocused" | "popupSpecificHotkeys"
  >;
export default function createKeydownListener(
  ...
  const undeferred = (e: KeyboardEvent): void => {
    ...
    if (eventTargetIsInput(e) && settings.popupDisableHotkeysWhileInputFocused)
      return;
