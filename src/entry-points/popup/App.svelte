...
<script lang="ts">
  ...
  async function getTab() {
    ...
    const tabs = await browserOrChrome.tabs.query({
      active: true,
      currentWindow: true,
    });
    return tabs[0];
  ...
  const tabLoadedPromise = (async () => {
    ...
    if (tab.status !== "complete") {
      // TODO it says `status` is optional? When is it missing?
        ...
        async function queryTabStatusAndScheduleAnotherIfNotFinished() {
          ...
          if (!finished) {
            pollTimeout = setTimeout(
              queryTabStatusAndScheduleAnotherIfNotFinished,
              2000,
            );
          }
        }
  ...
  const firstTelemetryReceivedP = new Promise<void>(
    (r) => (resolveFirstTelemetryReceivedP = r),
  );
  ...
  let keydownListener:
    ReturnType<typeof createKeydownListener> | (() => {}) = () => {};
  (async () => {
    ...
    const onMessageListener = (
      ...
      if (
        ...
        (!elementLastActivatedAt ||
          message.elementLastActivatedAt > elementLastActivatedAt)
      ) {
        disconnect?.();
        ...
        const telemetryPort = browserOrChrome.tabs.connect(tab.id!, {
          name: "telemetry",
          frameId,
        });
        ...
        (function sendGetTelemetryAndScheduleAnother() {
          ...
          telemetryTimeoutId = setTimeout(
            sendGetTelemetryAndScheduleAnother,
            telemetryUpdatePeriod * 1000,
          );
        })();

        nonSettingsActionsPort = browserOrChrome.tabs.connect(tab.id!, {
  ...
  const throttledSaveUnsavedSettingsToStorageAndTriggerCallbacks = throttle(
    () => {
      ...
      thisScriptRecentlyUpdatedStorageTimeoud = (
  ...
  function updateSettingsLocalCopyAndStorage(
    newValues: Partial<typeof settings>,
  ) {
    assignNewSettings(newValues);
    Object.keys(newValues).forEach((key) =>
      settingsKeysToSaveToStorage.add(key as keyof typeof newValues),
    );
  ...
  const tippyThemeMyTippyAndPreLine =
    tippyThemeMyTippy + " white-space-pre-line";

  $: silenceSpeedLabelClarification =
  ...
  function onUseExperimentalAlgorithmInput(e: Event) {
    const newControllerType = (e.target as HTMLInputElement).checked
      ? ControllerKind_CLONING
      : ControllerKind_STRETCHING;
    const newValues = changeAlgorithmAndMaybeRelatedSettings(
      settings,
      newControllerType,
    );
  ...
  $: controllerTypeAlwaysSounded =
    latestTelemetryRecord?.controllerType === ControllerKind_ALWAYS_SOUNDED;

  const displayNewBadgeOnExperimentalAlgorithm =
    new Date() < new Date("2024-09-30");
  ...
  function onSimpleSliderInput() {
    ...
    updateSettingsLocalCopyAndStorage({
      ...
      silenceSpeedRaw: 1.5 + settings.simpleSlider * 0.02,
      marginAfter: 0.03 + 0.002 * (100 - settings.simpleSlider),
    });
  }
  ...
  settingsPromise.then((settings) => {
    ...
    [...settings.hotkeys, ...settings.popupSpecificHotkeys].forEach(
      (hotKey) => {
  ...
  function getActionString(actionId: HotkeyAction, actionName: string): string {
    ...
    actionHotkeys.forEach((hotkey: HotkeyBinding, i: number) => {
      ...
      actionString +=
        keysString + hotkey.keyCombination.code.replace(/^Key/, "");
  ...
  (async () => {
    ...
    const remainingDuration =
      latestTelemetryRecord.elementRemainingIntrinsicDuration;
...
{#await settingsPromise then _}
      ...
      {#if settings.advancedMode}
        <div style="margin-bottom: 0.375rem;">
          ...
          <button
            ...
            style="padding: 0;">⚙️</button
          >
        </div>
      {/if}
      <div style="margin-bottom: 0.375rem;">
        {#await import(/* webpackExports: ['default'] */ /* webpackPreload: true */ "./TimeSaved.svelte")}
  ...
  <!-- TODO transitions? -->
  <div
    style={`--popupChartWidth: ${settings.popupChartWidthPx}px;` +
  ...
  >
            ...
            {#if considerConnectionFailed}
                  ...
                  {#if settings.applyTo !== "both"}
                    <button
                      ...
                      >🔍 {getMessage(
                        "alsoSearchFor",
                        getMessage(
                          settings.applyTo === "videoOnly" ? "audio" : "video",
                        ),
                      )}</button
                    >
                  {/if}
              ...
              could be useful. -->
                  <button
                    ...
                    style="margin: 0.25rem">🔄 {getMessage("retry")}</button
                  >
                </p>
              {:else}
                <p>
                  ...
                  {#each getMessage("suggestOpenLocalFile", getMessage("openLocalFile")).split("**") as part, i}
                    ...
                    {:else}
                      ...
                      <a
                        ...
                        on:click={onClickOpenLocalFileLink}>{part}</a
                      >
                    {/if}
                  {/each}
                </p>
    ...
    the video when the popup is open. This does _not_ apply to all websites. -->
        {#await import(/* webpackExports: ['default'] */ "./Chart.svelte")}
          <div
            style={"min-width: var(--popupChartWidth);" +
              "min-height: var(--popupChartHeight);" +
    ...
    to produce sound, don't show the warning. -->
        {#if latestTelemetryRecord?.elementLikelyCorsRestricted}
          {#await import(/* webpackExports: ['default'] */ "./MediaUnsupportedMessage.svelte")}
            <!-- `await` so it doesnt get shown immediately so it doesn't flash -->
            {#await new Promise((r) => setTimeout(r, 300)) then _}
              ⏳ {getMessage("loading")}...
            {/await}
          {:then { default: MediaUnsupportedMessage }}
            <MediaUnsupportedMessage
              ...
              on:dontAttachToCrossOriginMediaChange={({ detail }) => {
                updateSettingsLocalCopyAndStorage({
                  dontAttachToCrossOriginMedia: detail,
                });
              }}
            />
          {/await}
  ...
  {:else}
    ...
    {#if // The opposite day mode only applies to the cloning controller.
    settings.experimentalControllerType === ControllerKind_CLONING && (settings.oppositeDayMode === OppositeDayMode_UNDISCOVERED ? oppositeDayModeIsDiscoverable : settings.oppositeDayMode !== OppositeDayMode_HIDDEN_BY_USER)}
      <br />
      <label
        style="margin-top: 1rem; display: inline-flex; align-items: center;"
      >
    ...
    />
  {/if}
  {#if settings.advancedMode || settings.onPlaybackRateChangeFromOtherScripts !== "updateSoundedSpeed"}
  ...
  {#if settings.advancedMode}
    <RangeSlider
      ...
      disabled={settings.experimentalControllerType ===
        ControllerKind_CLONING || controllerTypeAlwaysSounded}
      useForInputParams={{
        content: () => {
          let tooltip = getMessage(
            "silenceSpeedTooltip",
            settings.silenceSpeedSpecificationMethod ===
              "relativeToSoundedSpeed"
  ...
  >
    {#if settings.popupAlwaysShowOpenLocalFileLink}
      ...
      <a
        ...
        style="margin-top: 1rem;">📂 {getMessage("openLocalFile")}</a
      >
    {/if}
    <label
      style="
