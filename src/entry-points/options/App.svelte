...
<script lang="ts">
  ...
  import HotkeysTable, {
    PotentiallyInvalidHotkeyBinding,
  } from "./components/HotkeysTable.svelte";
  ...
  import { getDecayTimeConstant as getTimeSavedDataWeightDecayTimeConstant } from "@/entry-points/content/TimeSavedTracker";
  ...
  type PotentiallyInvalidSettingsChangedKeys = keyof Pick<
    Settings,
    "hotkeys" | "popupSpecificHotkeys"
  >;
  type PotentiallyInvalidSettings = Omit<
    Settings,
    PotentiallyInvalidSettingsChangedKeys
  > & {
  ...
  Promise.all([settingsPromise, originalSettingsPromise]).then(
    () => (initialized = true),
  );

  // `commands` API is currently not supported by Gecko for Android.
  const commandsPromise:
    undefined | ReturnType<typeof browserOrChrome.commands.getAll> =
    browserOrChrome.commands?.getAll?.();

  function checkValidity(
    settings: PotentiallyInvalidSettings,
  ...
  function saveSettings() {
    assertDev(
      ...
      "Expected saveSettings to be called only when the form is valid",
    );
  ...
  window.addEventListener("beforeunload", (e) => {
    if (formEl.checkValidity()) {
      ...
      e.preventDefault();
      e.returnValue = "";
      return ""; // Some polyfilling right here.
    }
  });

  const onPlaybackRateChangeFromOtherScriptsOptions: Array<{
    v: Settings["onPlaybackRateChangeFromOtherScripts"];
    l: string;
  }> = [
    {
      v: "updateSoundedSpeed",
      l: `= ${getMessage("updateSoundedSpeedWheneverItChangesOnWebsite")}`,
    },
    {
      v: "prevent",
      l: `✋ ${getMessage("preventOtherScriptsFromChangingPlaybackRate")}`,
    },
    {
      v: "doNothing",
      l: `🧘 ${getMessage("doNothingWheneverPlaybackRateChanges")}`,
    },
  ];
  const silenceSpeedSpecificationMethodOptions: Array<{
    v: Settings["silenceSpeedSpecificationMethod"];
    l: string;
  }> = [
    { v: "relativeToSoundedSpeed", l: `✖️ ${getMessage("relativeToSounded")}` },
    {
      v: "absolute",
      l: `= ${getMessage("absolute")}${getMessage("absoluteSilenceSpeedClarification")}`,
    },
  ];
  const badgeWhatSettingToDisplayByDefaultOptions: Array<{
    v: Settings["badgeWhatSettingToDisplayByDefault"];
    l: string;
  }> = [
  ...
  const timeSavedAveragingMethodOptions: Array<{
    v: Settings["timeSavedAveragingMethod"];
    l: string;
  }> = [
    { v: "all-time", l: `♾️ ${getMessage("timeSavedAveragingMethodAllTime")}` },
    {
      v: "exponential",
      l: `📉 ${getMessage("timeSavedAveragingMethodExponential")}`,
    },
  ];
  const popupChartSpeedOptions: Array<{
    v: Settings["popupChartSpeed"];
    l: string;
  }> = [
    { v: "intrinsicTime", l: `▶️= ${getMessage("chartSpeedIntrinsicTime")}` },
    {
      v: "soundedSpeedTime",
      l: `▶️➗ ${getMessage("chartSpeedSoundedSpeedTime")}`,
    },
  ...
  switch (BUILD_DEFINITIONS.BROWSER) {
    ...
    case "gecko":
      ...
      editNativeShortcutsLinkUrl =
        "https://support.mozilla.org/kb/manage-extension-shortcuts-firefox";
      break;
    default:
      assertNever(BUILD_DEFINITIONS.BROWSER);
  }
  ...
  async function downloadFromSync() {
    Object.assign(
      settings,
      (await browserOrChrome.storage.sync.get()) as Partial<Settings>,
    );
    settings = settings;
  }
  async function uploadToSync() {
    ...
    browserOrChrome.storage.sync.set(
      filterOutLocalStorageOnlySettings(settings),
    );
  }

  const snowflakeExtensionUrl =
...
<main>
  {#await settingsPromise then _}
    {#if initialized}
      <form bind:this={formEl} on:submit|preventDefault={saveSettings}>
        <section>
          ...
          <fieldset>
            ...
            {#each onPlaybackRateChangeFromOtherScriptsOptions as { v, l }}
              ...
              />
              <label for={`onPlaybackRateChangeFromOtherScripts-radio-${v}`}
                >{l}</label
              >
              <br />
            {/each}
          </fieldset>
          <InputFieldBase label={getMessage("applyTo")} let:id>
            <select {id} bind:value={settings.applyTo} required>
              {#each [{ v: "videoOnly", l: `🎥 ${getMessage("applyToOnly", getMessage("video"))}` }, { v: "audioOnly", l: `🔉 ${getMessage("applyToOnly", getMessage("audio"))}` }, { v: "both", l: `🎥&🔉 ${getMessage("applyToBoth")}` }] as { v, l }}
        ...
        until that changes. -->
          <CheckboxField
            ...
            bind:checked={
              settings.useSeparateMarginSettingsForDifferentAlgorithms
            }
          />
        </section>
        <section>
                    ...
                    <tr>
                      ...
                      <td
                        >{command.name === "_execute_action"
                          ? getMessage("openPopup")
                          : command.description}</td
                      >
                      <td>
                        <input
                          disabled
        ...
        <section>
          ...
          <section>
            ...
            />
            <!-- ▶️ -->
            <InputFieldBase label="🚶 {getMessage('chartSpeed')}" let:id>
              <select {id} bind:value={settings.popupChartSpeed}>
            ...
            <div style="overflow-x: auto;">
              <table style="margin: 0.75rem 0;">
                <thead>
                  ...
                  {#each [getMessage("min"), getMessage("step"), getMessage("max")] as l}
                    <th>{l}</th>
                  {/each}
                </thead>
                <tbody>
                      ...
                      {#each rangeInputAttrs as attr}
                        ...
                        Or use a checkbox that sets it to "any"? -->
                          <input
                            ...
                            bind:value={
                              settings[
                                `popup${rangeInputSettingNameCapitalized.v}${attr}`
                              ]
                            }
                          />
                        </td>
          ...
          {#if settings.enableHotkeys}
            <!-- TODO Are you sure this needs to be hidden? -->
            <CheckboxField
          ...
          {#if settings.oppositeDayMode !== OppositeDayMode_UNDISCOVERED}
            ...
            <CheckboxField
              ...
              checked={settings.oppositeDayMode !==
                OppositeDayMode_HIDDEN_BY_USER}
              on:change={(e) => {
        ...
        <section>
          ...
          <InputFieldBase
            ...
            let:id
          >
            <select {id} bind:value={settings.timeSavedAveragingMethod}>
        ...
        <section>
          ...
          <button type="button" style="color: red;" on:click={downloadFromSync}
            >📥 {getMessage("downloadFromSync")}</button
          >
          <br /><br />
          <button type="button" disabled={!formValid} on:click={uploadToSync}
  ...
  {#if contactEmailHref}
    <div style="margin: 1rem 0;">
      <a
        ...
        rel="extenral noopener noreferrer">📧 {getMessage("contact")}</a
      >
    </div>
  {/if}
  <div style="margin: 1rem 0;">
    <a
      ...
      rel="extenral noopener noreferrer">💬 {getMessage("chat")}</a
    >
  </div>
  <div style="margin: 1rem 0;">
    <a
      ...
      rel="extenral noopener noreferrer">🌐 {getMessage("helpTranslate")}</a
    >
  </div>
  <div style="margin: 1rem 0;">
    <a
      ...
      rel="extenral noopener noreferrer">💸 {getMessage("donate")}</a
    >
  </div>
  ...
  <div style="margin: 1rem 0;">
    <a target="_blank" href={browserOrChrome.runtime.getURL("/license.html")}
  ...
  <div style="margin: 1rem 0;">
    <a
      ...
      rel="extenral noopener noreferrer">ℹ️ {getMessage("about")}</a
    >
  </div>
</main>
<footer>
...
<div class="status-bar">
  ...
  <p
    style="min-height: 2rem; opacity: 0.8; display: flex; margin: 0; align-items: center;"
  >
    ...
    {#if unsaved}
      ...
      {:else}
        <span>
          ...
          <button type="button" on:click={(_) => formEl.reportValidity()}
