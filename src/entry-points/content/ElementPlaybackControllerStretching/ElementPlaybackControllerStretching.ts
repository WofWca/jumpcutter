...
const MAX_MARGIN_BEFORE_REAL_TIME =
  MAX_MARGIN_BEFORE_INTRINSIC_TIME / MIN_SPEED;
...
type ControllerInitialized = Controller & { initialized: true } & Required<
    Pick<
      ...
      | "initialized"
      | "audioContext"
      | "_silenceDetectorNode"
      | "_analyzerIn"
      | "_volumeInfoBuffer"
      | "_lastActualPlaybackRateChange"
    >
  >;
type ControllerWithStretcher = Controller &
  Required<Pick<Controller, "_lookahead" | "_stretcherAndPitch">>;
type ControllerLogging = Controller &
  Required<Pick<Controller, "_log" | "_analyzerOut">>;

// Not a method so it gets eliminated at optimization.
const isLogging = (controller: Controller): controller is ControllerLogging =>
  logging;
...
export default class Controller {
  ...
  async init(): Promise<this> {
    ...
    if (isLogging(this)) {
      outVolumeFilterP = volumeFilterProcessorP.then(() => {
        ...
        this._destroyedPromise.then(() =>
          destroyAudioWorkletNode(outVolumeFilter),
        );
    ...
    if (this.isStretcherEnabled()) {
      ...
      const StretcherAndPitchCorrectorNode = (
        await import(
          /* webpackExports: ['default'] */
          "./StretcherAndPitchCorrectorNode"
        )
      ).default;
      const maxSpeedToPreserveSpeech =
        audioContext.sampleRate / MIN_HUMAN_SPEECH_ADEQUATE_SAMPLE_RATE;
      const maxMaginStretcherDelay =
        MAX_MARGIN_BEFORE_REAL_TIME * (maxSpeedToPreserveSpeech / MIN_SPEED);
    ...
    let toDestinationChainLastConnectedLink: {
    ...
    toAwait.push(
      silenceDetectorP.then((silenceDetector) => {
        silenceDetector.port.onmessage = ({
          data,
        }: MessageEvent<SilenceDetectorMessage>) => {
          ...
          if (silenceStartOrEnd === SilenceDetectorEventType.SILENCE_END) {
            ...
            if (
              browserHasAudioDesyncBug &&
              this.settings.enableDesyncCorrection
            ) {
              // A workaround for
          ...
          if (IS_DEV_MODE) {
            ...
            if (delayS > 0.03) {
              console.warn(
                `Skipped too much, or too little: delay: ${delayS}s`,
              );
            }
          }
        };
  ...
  private _setStateAccordingToNewSettings(
    ...
    this._silenceDetectorNode.durationThreshold =
      this._getSilenceDetectorNodeDurationThreshold();
    if (this.isStretcherEnabled()) {
  ...
  get telemetry(): TelemetryRecord {
    ...
    const inputVolume =
      this._volumeInfoBuffer[this._volumeInfoBuffer.length - 1];
    ...
    const stretchToInputTime = (stretch: StretchInfo): StretchInfo => ({
      ...
      endTime:
        stretch.endTime -
        getDelayFromInputToStretcherOutput(lookaheadDelay, stretch.endValue),
    });
    ...
    return {
      ...
      delayFromInputToStretcherOutput: getDelayFromInputToStretcherOutput(
        lookaheadDelay,
        stretcherDelay,
      ),
      stretcherDelay,
