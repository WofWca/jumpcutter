...
import type { GlobalThisWithBridgeElement } from "@/entry-points/content/cloneMediaSources/main-for-extension-world";
import {
...
const bridgeElement = (globalThis as GlobalThisWithBridgeElement)[
  BRIDGE_ELEMENT_ID_AND_PROP_NAME
];
...
export async function getMediaSourceCloneElement(
  ...
  return new Promise((r_) => {
    ...
    const scheduleResolvePromiseAndRemoveListener = (
      ...resolveParams: Parameters<typeof r_>
    ) => {
      ...
      bridgeElement.removeEventListener(
        GET_CLONE_RESPONSE_EVENT_NAME,
        listener,
      );
    };

    bridgeElement.addEventListener(GET_CLONE_RESPONSE_EVENT_NAME, listener, {
      passive: true,
    });
    ...
    originalEl.dispatchEvent(
      new CustomEvent(GET_CLONE_REQUEST_EVENT_NAME, {
        bubbles: true,
