...
class SilenceDetectorProcessor extends WorkaroundAudioWorkletProcessor {
  ...
  isPastDurationThreshold(
    saimpleInd: number,
    durationThresholdSamples: number,
  ) {
  ...
  process(
    ...
    if (input.length === 0) {
      if (!assumeSoundedWhenUnknown) {
        throw new Error(
          "The below code assumes video parts to be sounded when it is unknown",
        );
      }
    ...
    if (IS_DEV_MODE) {
      ...
      if (numChannels !== 1 && !devErrorShown) {
        ...
        console.error(
          "SilenceDetectorProcessor assumes that there's only one channel on its input",
        );
        devErrorShown = true;
      }
    }
    ...
    const durationThresholdSamples =
      parameters.durationThreshold[0] * sampleRate;
