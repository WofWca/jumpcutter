...
function tryAddVolumeThresholdHotkeys(hotkeys: HotkeyBinding[]): boolean {
  ...
  function bindingIsEqual(a: HotkeyBinding, b: HotkeyBinding) {
    return (
      ...
      (a.overrideWebsiteHotkeys ?? false) ===
        (a.overrideWebsiteHotkeys ?? false)
    );
  }
  const defaultVolumeThresholdHotkeysPresent =
    defaultVolumeThresholdHotkeys.every((defaultBinding) =>
      hotkeys.some((binding) => bindingIsEqual(binding, defaultBinding)),
    );
  const newHotkeysAreAlreadyBound = hotkeys.some(
    ({ keyCombination: existing }) => {
      return newHotkeys.some(({ keyCombination: newCombination }) =>
        combinationIsEqual(newCombination, existing),
      );
    },
  );
  if (defaultVolumeThresholdHotkeysPresent && !newHotkeysAreAlreadyBound) {
    const insertAfter = hotkeys.findIndex((b) =>
      bindingIsEqual(b, defaultVolumeThresholdHotkeys[1]),
    );
...
export default async function (): Promise<void> {
  ...
  const { hotkeys } = (await browserOrChrome.storage.local.get("hotkeys")) as {
    hotkeys?: HotkeyBinding[];
  };
  if (hotkeys) {
