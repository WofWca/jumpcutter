...
import {
  settingKeyToPreviousValueKey,
  Settings,
  togglableSettings,
  TogglableSettings,
  CorrespondingPreviousValueSetting,
} from "@/settings";
import {
  clamp,
  assertNever,
...
export function keydownEventToActions(
  ...
  currentSettings: Pick<
    ...
    | "hotkeys"
    | TogglableSettings
    | CorrespondingPreviousValueSetting<TogglableSettings>
  >,
  ...
  const matchedBindings = bindingsDefinite.filter((binding) =>
    eventMatchesCombination(e, binding.keyCombination),
  );
  if (!matchedBindings.length) {
    return;
  }
  const settingsNewValues: Exclude<
    ReturnType<typeof keydownEventToActions>,
    undefined
  >[0] = {};
  const nonSettingsActions: Exclude<
    ReturnType<typeof keydownEventToActions>,
    undefined
  >[1] = [];
  ...
  for (const binding of matchedBindings) {
    ...
    const toggleSettingValue = (key: TogglableSettings) => {
      ...
      settingsNewValues[key] =
        currentSettings[key] === arg
    ...
    switch (binding.action) {
      ...
      case HotkeyAction.INCREASE_VOLUME_THRESHOLD:
        updateClamped("volumeThreshold", 1, 0, 1);
        break;
      case HotkeyAction.DECREASE_VOLUME_THRESHOLD:
        updateClamped("volumeThreshold", -1, 0, 1);
        break;
      case HotkeyAction.SET_VOLUME_THRESHOLD:
        settingsNewValues.volumeThreshold = arg;
        break;
      case HotkeyAction.TOGGLE_VOLUME_THRESHOLD:
        toggleSettingValue("volumeThreshold");
        break;
      case HotkeyAction.INCREASE_SOUNDED_SPEED:
        updateClamped(
          "soundedSpeed",
          1,
          binding.actionArgument!,
          maxSpeedClamp,
        );
        break;
      case HotkeyAction.DECREASE_SOUNDED_SPEED:
        ...
        break;
      case HotkeyAction.SET_SOUNDED_SPEED:
        settingsNewValues.soundedSpeed = arg;
        break;
      case HotkeyAction.TOGGLE_SOUNDED_SPEED:
        toggleSettingValue("soundedSpeed");
        break;
      ...
      case HotkeyAction.DECREASE_SILENCE_SPEED:
        ...
        break;
      case HotkeyAction.SET_SILENCE_SPEED:
        settingsNewValues.silenceSpeedRaw = arg;
        break;
      case HotkeyAction.TOGGLE_SILENCE_SPEED:
        toggleSettingValue("silenceSpeedRaw");
        break;
      case HotkeyAction.INCREASE_MARGIN_BEFORE:
        updateClamped("marginBefore", 1, 0, 1);
        break;
      case HotkeyAction.DECREASE_MARGIN_BEFORE:
        updateClamped("marginBefore", -1, 0, 1);
        break;
      case HotkeyAction.SET_MARGIN_BEFORE:
        settingsNewValues.marginBefore = arg;
        break;
      case HotkeyAction.TOGGLE_MARGIN_BEFORE:
        toggleSettingValue("marginBefore");
        break;
      case HotkeyAction.INCREASE_MARGIN_AFTER:
        updateClamped("marginAfter", 1, 0, 10);
        break;
      case HotkeyAction.DECREASE_MARGIN_AFTER:
        updateClamped("marginAfter", -1, 0, 10);
        break;
      case HotkeyAction.SET_MARGIN_AFTER:
        settingsNewValues.marginAfter = arg;
        break;
      case HotkeyAction.TOGGLE_MARGIN_AFTER:
        toggleSettingValue("marginAfter");
        break;
      default: {
        if (IS_DEV_MODE) {
          if (
  ...
  for (const key_ of Object.keys(settingsNewValues)) {
    ...
    if ((togglableSettings as any).includes(key)) {
      ...
      const prevValueSettingKey =
        settingKeyToPreviousValueKey[key as TogglableSettings];
